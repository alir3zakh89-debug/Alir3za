...
BOT_TOKEN = "8227905527:AAGdPY0JB9t5Z2OaJSCOq_jWqT7oJIz8DDQ" # توکن ربات 
ADMINS = [8368059469]  # ایدی ادمین چند تا هم میتونید بزارید
...
async def show_required_chats_list(update: Update, context: ContextTypes.DEFAULT_TYPE, page: int = 1):
    ...
    message_text = f"{EMOJIS['channel']} <b>لیست چت‌های جوین (صفحه {page} از {total_pages})</b>\n\n"
    
    if not page_chats:
        message_text += "هیچ چت یافت نشد."
    else:
...
async def handle_chat_info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(
            f"{EMOJIS['check']} چت @{username} ({chat_type}) با موفقیت به لیست جوین اضافه شد.",
            parse_mode='HTML'
        )
    except Exception as e:
...
"""
        ...
        if not chats:
            ...
            message_text += "هیچ چت تنظیم نشده است.\n"
        else:
            for chat in chats:
                chat_type = "کانال" if chat['chat_type'] == 'channel' else "گروه"
